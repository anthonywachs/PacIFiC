...
void GrainsTestDev::Simulation( double time_interval )
{
  if ( m_processorIsActive )
  {
    ...
    // Construct two RigidBodyWithCrust objects
    Convex* convexA = new Box( 1., 1., 16. );
    Transform const* trA = new Transform( 0., 0., 0. );
    // RigidBodyWithCrust RBWCa( convexA, trA );
    // RBWCa.setCrustThickness( 0.01 );

    Convex* convexB = new Box( 1., 1., 16. );
    Transform const* trB = new Transform( 1.5, 1.5, 0. );
    // RigidBodyWithCrust RBWCb( convexB, trB );
    // RBWCb.setCrustThickness( 0.01 );
    ...
    // Call GJK
    {
      ...
      int nbIterGJK = 0;
      // Transform const* a2w = RBWCa.getTransformWithCrust();
      // Transform const* b2w = RBWCb.getTransformWithCrust();
      auto start = high_resolution_clock::now();
      for ( ctr = 1; ctr <= 1e6 && cntct == false; ctr++ )
      {
      ...
      cout << "Contact " << cntct << ", Iter " << ctr <<
              ", Time taken by GJK: " << duration.count() << " us" << endl;
    }

    // Call analCyl
    {
      BCylinder bCylA = convexA->getBCylinder();
      BCylinder bCylB = convexB->getBCylinder();
