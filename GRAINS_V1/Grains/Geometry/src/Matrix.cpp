...
void Matrix::round( double tol )
{
  m_elem[X][X] = fabs( m_elem[X][X] ) < tol ? 0. : m_elem[X][X];
  m_elem[X][Y] = fabs( m_elem[X][Y] ) < tol ? 0. : m_elem[X][Y];
  m_elem[X][Z] = fabs( m_elem[X][Z] ) < tol ? 0. : m_elem[X][Z];
  m_elem[Y][X] = fabs( m_elem[Y][X] ) < tol ? 0. : m_elem[Y][X];
  m_elem[Y][Y] = fabs( m_elem[Y][Y] ) < tol ? 0. : m_elem[Y][Y];
  m_elem[Y][Z] = fabs( m_elem[Y][Z] ) < tol ? 0. : m_elem[Y][Z];
  m_elem[Z][X] = fabs( m_elem[Z][X] ) < tol ? 0. : m_elem[Z][X];
  m_elem[Z][Y] = fabs( m_elem[Z][Y] ) < tol ? 0. : m_elem[Z][Y];
  m_elem[Z][Z] = fabs( m_elem[Z][Z] ) < tol ? 0. : m_elem[Z][Z];
}
...
{
  return ( Matrix( c * m[X][X], c * m[X][Y], c * m[X][Z],
