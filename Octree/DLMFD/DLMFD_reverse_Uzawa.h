...
#if debugBD == 0   
#if DLM_Moving_particle
  reverse_fill_flagfield (p, flagfield_mailleur, index_lambda, 0, DLM_periodic_shift);
#endif
...
# if debugBD == 0
  ...
  for (int k = 0; k < NPARTICLES; k++) {
	...
	if (((int)index_lambda.x[] > -1) && (level == depth()) && is_leaf(cell) && ((int)index_lambda.y[] == pp->pnum)) {
	  ...
	  weight = reversed_weight (pp, weightcellpos, lambdacellpos, lambdapos, Delta, ppshift);
