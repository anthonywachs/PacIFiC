...
double reversed_weight (particle * pp, const coord weightcellpos, const coord lambdacellpos, const coord lambdapos, const double delta, const coord pshift) {
  ...
  if (goflag == 1) {
    weight = compute_weight_Quad (weight_id, lambdapos, lambdacellpos, NCX, CX, delta);
  }
    return weight;
}
...
void reverse_fill_flagfield (particle * p, scalar f, vector index_lambda, const int cacheflag, vector pshift) {
