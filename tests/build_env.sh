cd ../
sed -i "s|TO_BE_OVERWRITTEN|$(echo $PWD)|" ./Env/CI-RUNNER-PacIFiC-OpenMPI-2.1.1-GNU-8.2.1.env.sh
cd ./Env/
...
sed -i 's|${GRAINS_HOME}/XERCES-2.8.0|/media/data/softwares/XERCES-2.8.0|' GRAINS_V1/Env/grains_default.env.sh
sed -i 's|${GRAINS_HOME}/XERCES-2.8.0|/media/data/softwares/XERCES-2.8.0|' GRAINS_V1/Env/grains_default.env.csh
...
sed -i 's|HYPRE_DIR=${MACWORLD_ROOT}/hypre-2.10.1|HYPRE_DIR=/media/data/softwares/hypre-2.10.1|' ${PACIFIC_HOME}/Cartesian/MacWorld/extra_files/hypre.env.sh
sed -i 's|PETSC_DIR=${MACWORLD_ROOT}/petsc-${PETSC_VERSION_PATCH}|PETSC_DIR=/media/data/softwares/petsc-3.2.0-p7|' ${PACIFIC_HOME}/Cartesian/MacWorld/extra_files/petsc.env.sh
sed -i 's|export BASILISK=${OCTREE_HOME}/basilisk/src|export BASILISK=/media/data/softwares/basilisk/src|' ${PACIFIC_HOME}/Octree/Env/octree.env.sh
sed -i 's|export BASILISK_MESA_GLU_INSTALL=|export BASILISK_MESA_GLU_INSTALL=/media/data/softwares/local|' ${PACIFIC_HOME}/Octree/Env/octree.env.sh
