...
/* Set start
------------*/
void SolverComputingTime::SCT_set_start(const string &app_name_)       
{
  list<ComputingTime>::iterator il=all_apps.begin();
  bool found=false;
  
  while((il!=all_apps.end())&&(!found))
  {
  ...
  { 
    ...
    for (il=all_apps.begin();il!=all_apps.end();il++)
...
-----------------------*/
double SolverComputingTime::SCT_get_elapsed_time(const string &app_name_)
{
  list<ComputingTime>::iterator il=all_apps.begin();
  bool found=false;
...
--------------------------------------------------------*/
void SolverComputingTime::SCT_add_elapsed_time(const string &app_name_)
{
  list<ComputingTime>::iterator il=all_apps.begin();
  bool found=false;
...
-----------------------------*/
double SolverComputingTime::SCT_get_total_elapsed_time(const string &app_name_)
	const
{
  list<ComputingTime>::const_iterator il=all_apps.begin();
  bool found=false;
...
void SolverComputingTime::SCT_get_summary(ostream &f,
	const double &solver_total_time) const
{
  ...
  if (!all_apps.empty())
  {
    ...
    f << "----------------------------------------------------------" << 
    	"-------------------------" << endl;
    f << setw(35) << "Total" << setw(12) << solver_total_time << endl;
    f << "----------------------------------------------------------" << 
    	"-------------------------" << endl;    	
  }     
...
---------------------------*/
void SolverComputingTime::SCT_insert_app(const string &app_name_)
{
  list<ComputingTime>::iterator il=all_apps.begin();
  bool found=false;
