...
//----------------------------------------------------------------------
void
FV_DiscreteField_Staggered::synchronize_extended_DOF_constrained(
    size_t level, list<FV_TRIPLET> &ijk, list<size_t> &comps )
//----------------------------------------------------------------------
{
   MAC_LABEL( "FV_DiscreteField_Staggered::"
              "synchronize_extended_DOF_constrained" ) ;
   MAC_ASSERT( synchronized_level == level );
   
 
   list<FV_TRIPLET> ijk_periodic;
   list<size_t> comps_periodic;
   FV_TRIPLET mt ;
   mt.i = 0 ;
   mt.j = 0 ;   
   mt.k = 0 ; 
   list<FV_TRIPLET>::iterator imac ;
   list<size_t>::iterator icomp = comps.begin();
   size_t i, j, k, comp;


   // Add periodic (buffer) zone unknown numbering in lists
   for (imac=ijk.begin();imac!=ijk.end();imac++, icomp++)
   {
     i = imac->i ;
     j = imac->j ;
     k = imac->k ;
     ...
     if( (*DOFstatus)[comp](i,j,k) == FV_DOF_PERIODIC_BUFFERZONE || 
      	 (*DOFstatus)[comp](i,j,k) == FV_DOF_BUFFERZONE_PERIODIC_BUFFERZONE ||
      	 (*DOFstatus)[comp](i,j,k) == FV_DOF_PERIODIC_HALOZONE )
     {
       // X periodicity
       if ( (*PERIODIC)(0) && comp==0 )
       {
	 mt.i = i + (*local_min_index_in_global)[comp](0) 
	   + (*PERIODIC_SHIFT)[comp].i ;
	 mt.j = j + (*local_min_index_in_global)[comp](1) ;   
	 mt.k = DIM == 2 ? 0 : 
	   k + (*local_min_index_in_global)[comp](2) ;	       
	 if ( is_global_triplet( mt.i, mt.j, mt.k, comp ) )
	 {
	   ijk_periodic.push_back( mt ) ;
	   comps_periodic.push_back( comp ) ; 
	 }
	 ...
	 if ( is_global_triplet( mt.i, mt.j, mt.k, comp ) )	       
       ...
       // Y periodicity
       if ( (*PERIODIC)(1)  && comp==1 )
       {
	 mt.i = i + (*local_min_index_in_global)[comp](0) ;
	 mt.j = j + (*local_min_index_in_global)[comp](1) 
	   + (*PERIODIC_SHIFT)[comp].j ;   
	 mt.k = DIM == 2 ? 0 : 
	   k + (*local_min_index_in_global)[comp](2) ;	       
	 if ( is_global_triplet( mt.i, mt.j, mt.k, comp ) )	       
	 {
	 ...
	 if ( is_global_triplet( mt.i, mt.j, mt.k, comp ) )	       
       ...
       {
	 // Z periodicity
	 if ( (*PERIODIC)(2)  && comp==2)
	 {
	   mt.i = i + (*local_min_index_in_global)[comp](0) ;
	   mt.j = j + (*local_min_index_in_global)[comp](1) ;   
	   mt.k = k + (*local_min_index_in_global)[comp](2) 
	     + (*PERIODIC_SHIFT)[comp].k ;	       
	   if ( is_global_triplet( mt.i, mt.j, mt.k, comp ) )	       
	   {
	     ijk_periodic.push_back( mt ) ;
	     comps_periodic.push_back( comp ) ; 
   ...
   size_t width = 4 ;
   intVector global_num_comm( ijk_periodic.size()*width, 0 );
   size_t positionIndex = 0 ;
 
   list<FV_TRIPLET>::iterator imac_periodic;
   list<size_t>::iterator icomp_periodic = comps_periodic.begin();

   for (imac_periodic=ijk_periodic.begin();
	imac_periodic!=ijk_periodic.end();
	imac_periodic++, icomp_periodic++)
   {
      global_num_comm(positionIndex) = imac_periodic->i ;
      global_num_comm(positionIndex+1) = imac_periodic->j ;
      global_num_comm(positionIndex+2) = imac_periodic->k ;
      global_num_comm(positionIndex+3) = *icomp_periodic ;
      positionIndex += width ;  
   }
   ...
   list<size_t>::iterator r,q;
   for (r=MPI_periodic_neighbors_world.begin();
   	r!=MPI_periodic_neighbors_world.end();r++)
   {
     if ( *r == rank )
     {
       for (q=MPI_periodic_neighbors_world.begin();
       	q!=MPI_periodic_neighbors_world.end();q++)
         if ( *q != rank )
 	   macCOMM->send( *q, global_num_comm ) ; 
     }
     else
     {
       //receive global numbering
 	 ...
 	 if ( is_global_triplet_local_DOF( ig, jg, kg, comp ) ) 
 	 {
 	   (*DLMFDconstrainedDOFs)[comp]
 	     ( ig-(*local_min_index_in_global)[comp](0),
 	       jg-(*local_min_index_in_global)[comp](1),
 	       DIM == 2 ? 0 : kg - (*local_min_index_in_global)[comp](2) )
 	     = true;
 	 }	 	 
         positionIndex += width ;
       }
   ...
   // Add periodic (halozone) points to extended DOF list
   ijk.splice( ijk.end(), ijk_periodic ) ;
   comps.splice( comps.end(), comps_periodic ) ;
