...
bool GrainsMPI::insertParticule( const PullMode& mode )
{
  ...
  if ( insert_counter == 0 )
  {
    // En parallèle, si on insert des particules avec des positions predefinies
    ...
    if ( particule ) 
    {
      ...
      if ( m_position == "" ) 
      {
        // Afin que le tirage aléatoire soit le même sur tous les procs
      ...
      // situee et élimination sur les autres
	...
	bool const& random_local )
{
  ...
  switch (mode) 
  {
	...
	if ( il->second != 0 ) classesDispo.push_back(i);
      ...
      // En random global, afin que le tirage aléatoire soit le même sur tous 
...
void GrainsMPI::periodicParticles( bool b_perfTiming )
{
  ...
  // Création ou Mise à jour des clones periodiques
